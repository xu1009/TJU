...
u8 USART_RX_BUF5[USART_REC_LEN5];     //接收缓冲,最大USART_REC_LEN个字节.
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
...
void uart_init5(u32 bound5){
  //GPIO端口设置
  GPIO_InitTypeDef GPIO_InitStructure;
	USART_InitTypeDef USART_InitStructure;
	NVIC_InitTypeDef NVIC_InitStructure;
	 
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC|RCC_APB2Periph_GPIOD, ENABLE);	//使能USART2，GPIOA时钟
...
void UART5_IRQHandler(void)                	//串口1中断服务程序
	...
	if(USART_GetITStatus(UART5, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if((USART_RX_STA5&0x8000)==0)//接收未完成
			{
			if(USART_RX_STA5&0x4000)//接收到了0x0d
				{
				if(Res!=0x0a)USART_RX_STA5=0;//接收错误,重新开始
				else USART_RX_STA5|=0x8000;	//接收完成了 
				}
			else //还没收到0X0D
				{	
				if(Res==0x0d)USART_RX_STA5|=0x4000;
				else
					{
					USART_RX_BUF5[USART_RX_STA5&0X3FFF]=Res ;
					USART_RX_STA5++;
					if(USART_RX_STA5>(USART_REC_LEN5-1))USART_RX_STA5=0;//接收数据错误,重新开始接收	  
					}		 
				}
			}   		 
     } 
